...
TEST_CASE("wire_pack produces 3 frames")
{
    WireMessage msg;
    ...
    // Frame 1: header size
    CHECK(frames[1].size() == sizeof(WireHeader));

    // Frame 2: payload
    CHECK(frames[2] == msg.payload);
}
...
{
    Frames frames(2); // only 2 frames, need 3
    WireMessage out;
...
TEST_CASE("wire_unpack rejects wrong header size")
{
    Frames frames(3);
    frames[0] = { 'a' };
    frames[1] = { 0x01, 0x02 }; // too short for WireHeader
    frames[2] = {};
    WireMessage out;
    CHECK(wire_unpack(frames, out) == BACARO_EINVAL);
}
